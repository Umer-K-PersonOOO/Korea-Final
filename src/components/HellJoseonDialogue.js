...
const dialogueScript = [
  ...
  "Housing prices in this country are terrible — how can people say that we can buy our own homes as long as we save money?",
  "Even with a good job, owning property feels impossible. Many in my generation feel trapped.",
  "We call it 'Hell Joseon' — a society where status and wealth are out of reach for ordinary young people."
];

const hints = [
  {
    player: "Is saving not enough to buy a home anymore?",
    identity:
      "No matter how much we save, real estate prices rise faster. It's not realistic for most young people."
  },
  {
    player: "Why do you call it Hell Joseon?",
    identity:
      "Because it feels like a hopeless, stratified system where success is only possible if you are born into wealth."
  }
];

export default function HellJoseonDialogue({ onGuess, onExit }) {
  ...
  useEffect(() => {
    if (!isTyping && outOfLines && !hasWon && !hintTyping && !isWaiting) {
      setChatHistory((prev) => [
        ...
        {
          speaker: "identity",
          text: "Do you not understand the despair my generation faces?"
        }
      ]);
    }
  ...
  const submitChoice = (choice) => {
    ...
    if (
      choice.toLowerCase().includes("housing") ||
      choice.toLowerCase().includes("hell joseon") ||
      choice.toLowerCase().includes("real estate")
    ) {
      setChatHistory((prev) => [
        ...prev,
        {
          speaker: "identity",
          text:
            "Yes... the despair over housing and social stratification in 'Hell Joseon'.\n\n" +
            "- Park Su-ah, 2023 interview"
        },
      ...
      setChatHistory((prev) => [
        ...
        {
          speaker: "identity",
          text: "That is not the struggle I was describing."
        }
      ]);
    }
  };
  ...
  const submitGuess = async () => {
    ...
    if (
      answer.includes("hell joseon") ||
      answer.includes("housing") ||
      answer.includes("real estate")
    ) {
      setChatHistory((prev) => [
        ...prev,
        {
      ...
      >
                ...
                if (!usedSpecial) {
                  ...
                  setChatHistory((prev) => [
                    ...prev,
                    {
                      ...
                      isRecord: true,
                      text: {
                        title: "Youth Testimony - 2023",
                        body:
                          "Park Su-ah describes how real estate costs have crushed the dreams of young Koreans. The 'Hell Joseon' mindset reflects deep frustrations with class inequality and housing barriers.",
                        footer: "Business Insider Interview, 2023"
                      }
                    }
                  ]);
                  setIsWaiting(true);
        ...
        {outOfLines && !hasWon && (
          <div className="flex flex-col gap-2 mt-4 text-lg">
            <span className="text-gray-300">Choose what struggle I described:</span>
