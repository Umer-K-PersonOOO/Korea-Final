...
const dialogueScript = [
  "In Korea these days, if you want to survive on a monthly salary, it has to be with a large corporation or certain jobs.",
  "For normal people, having two jobs is a necessity if you want to save for the future.",
  "I work as a web designer during the day and at convenience stores at night.",
  "Over 100 hours a week. It’s exhausting. But this is the only way forward.",
  "My dream is to achieve financial freedom by the time I turn 40 or 45."
];

const hints = [
  {
    player: "Why do you work so much?",
    identity:
      "Because in this economy, working one job just isn’t enough anymore. If you don’t hustle, you fall behind."
  },
  {
    player: "Why not slow down and enjoy life?",
    identity:
      "If I stop now, I may never be able to own a home or retire comfortably. Sacrifice now, freedom later."
  }
];

export default function SpecGenerationDialogue({ onGuess, onExit }) {
  ...
  const submitChoice = (choice) => {
    ...
    if (choice.toLowerCase().includes("spec") || choice.toLowerCase().includes("individualism")) {
      setChatHistory((prev) => [
        ...prev,
        {
          speaker: "identity",
          text:
            "Yes... I represent the 'Spec' Generation.\n\n" +
            "We are the youth who hustle endlessly in pursuit of financial freedom, pressured by a system that tells us to work harder or fall behind.\n\n" +
            "- Kim Jaram, working professional"
        },
  ...
  const submitGuess = async () => {
    ...
    if (answer.includes("spec") || answer.includes("individualism")) {
      setChatHistory((prev) => [
        ...prev,
        {
          speaker: "identity",
                ...
                if (!usedSpecial) {
                  ...
                  setChatHistory((prev) => [
                    ...prev,
                    {
                      ...
                      isRecord: true,
                      text: {
                        title: "Interview, CNA Insider 2023",
                        body:
                          "Kim Jaram, a 32-year-old worker, describes juggling multiple jobs as a necessity for Korea's 'Spec' generation.",
                        footer: "channelnewsasia.com"
                      }
                    }
                  ]);
